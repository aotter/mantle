...
| [`aotter/mantle-starters`](https://github.com/aotter/mantle-starters) | End-user starters monorepo. Contains `_common/` (AGENTS.md, repo-local skills, scripts), `publication`, `blank`, etc. Also hosts `packages/create-mantle` — the npx scaffolder that fetches the starters tarball, merges `_common/` + `<archetype>/`, fills `{{PLACEHOLDER}}` macros, writes `.mantle/launch-state.json`, and prints RUN_NOTES JSON. **The scaffolder is intentionally not on npm**; consumers run it via `npx https://github.com/aotter/mantle-starters/releases/download/<tag>/aotter-create-mantle-<tag>.tgz` per `skills/install/SKILL.md`. Premium / per-customer starters live in the private sibling [`aotter/mantle-starters-premium`](https://github.com/aotter/mantle-starters-premium). |
