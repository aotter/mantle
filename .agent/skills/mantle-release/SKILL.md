...
Published versions are immutable. Once an npm package, GitHub tag, GitHub release, or starter tarball is public, do not force-retag, overwrite, or pretend the version can be repaired in place.
...
2. Confirm all Mantle package versions are aligned to the intended version unless an ADR explicitly permits divergence.
...
5. Check downstream readiness before tagging the SDK.
   ...
   In `../mantle-starters`, confirm any starter content required by the release is already merged to `develop`, especially `_common/`, `packages/create-mantle`, `.agent/`, `.claude/`, and feature overlays.

   ```bash
   git -C ../mantle-starters status --short --branch
   pnpm --dir ../mantle-starters/packages/create-mantle test
...
3. `aotter/mantle-starters` tags `vX.Y.Z` and creates the starter release tarball.
...
Download the starter tarball from the GitHub release and scaffold at least one starter. Verify generated repos include repo-local agent skills:
...
```bash
curl -fsSL 'https://mantle.tools/skill/install?...' | rg 'vX\\.Y\\.Z|aotter-create-mantle\\.tgz'
```
