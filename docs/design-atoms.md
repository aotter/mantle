...
| `update` | UPDATE in place. Merges patch with existing data via `projectUpdateAndStamp`. Bumps version. Caller `expectedVersion` is the **observed** native `entry.version` at read time (not `version+1`). | `input.id` (strict string) and `input.expectedVersion` (strict number) are required in `input.required`. |
| `upsert` | If `match` is declared, queries by matched natural key fields. Found: update using the **caller's** `expectedVersion` (never `preloaded.version`). Not found: create only when the caller omitted `expectedVersion`; a versioned write for a missing row is `NOT_FOUND`. If `match` is omitted (legacy), the same create/update intent rules apply to `input.id`. Schema `spec.schema.properties` must not declare `expectedVersion`. | If `match` is set, matched fields must match a declared unique index, be in `input.properties` and `input.required`; `id` must NOT be in `input`; `expectedVersion` MUST be declared as strict `number` but is not globally required (create has no version). If `match` is omitted, `expectedVersion` must still be declared as strict `number`; `id` is optional strict string. |
