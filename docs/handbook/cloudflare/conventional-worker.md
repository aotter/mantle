...
- Admin at `/admin` when Admin assets are present, OAuth consent and discovery, and MCP at `/mcp` and `/mcp/staff`. The MCP transport is Streamable HTTP served by the official SDK through `@aotter/mantle-mcp`, for both the 2026-07-28 per-request era and stateless 2025-era requests. There is no session header and the 2025-era `GET` stream answers `405`. A `POST` must send `Accept: application/json, text/event-stream`, otherwise it gets `406`, and its answer may be one SSE event rather than a JSON body. `tools/call` answers `401` with an OAuth challenge so a client can authenticate mid-session and retry. Verified against the official TypeScript client SDK `@modelcontextprotocol/client` 2.1.0 in the adapter's conformance test; that pin is the gate when the SDK is upgraded.
