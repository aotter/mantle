...
Admin's in-browser WebMCP tools are not a remote `/mcp/staff` server. The runnable reference mounts an anonymous, **read-only** `/api/mcp` backed only by public Views. On the public experimental Site, unauthenticated HTTPS requests to `POST /api/mcp` passed `initialize`, `tools/list`, and a published View `tools/call`. `POST /mcp` returned a different 404 before reaching the application's handler in this deployment; official Sites documentation does not establish that `/mcp` is a reserved path, so the reference uses the tested alternate path. This proves the manual HTTPS MCP transport works for public tools; it does **not** establish ChatGPT connector registration or staff OAuth. The reference does not mount staff MCP or issue OAuth tokens. Pass `mcpEndpoints: { public: "/api/mcp", staff: null }` to `mountMantleAdmin` with a Mantle version that supports this option, so Admin displays only the mounted endpoint. There are two distinct connection paths: Sites-provisioned connection details (`get_site` with `include_mcp_connection`) require a deployed MCP declaration, while a manually configured remote connector targets the HTTPS endpoint directly. Do not treat failure of the former as proof that the latter is impossible.
