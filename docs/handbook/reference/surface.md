...
| `preview_entry` | staff, App only | Only with a site renderer (on Cloudflare, mounted public routes) and an App resource that lists it in `appOnly`, as `interactionAppResource()` does; never offered to a model. `collection` is limited to the rendered collections. Returns `{ html }` in `structuredContent`, drafts included; `readOnlyHint: true`; contributor role and above. |
