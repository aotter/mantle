...
| `VIEW_TTL_NATIVE_UNSAFE` | A native SQL View names a TTL Schema's table, so it cannot guarantee logical expiry. `value` lists those Schemas. | — |
...
| `MANTLE_REF_INVALID` | An object-form `x-mantle-ref` is malformed, names an unknown Schema, or names a `field` that is neither `id` nor a single-field unique index of that Schema. | — |
| `PROCEDURE_TARGET_INVALID` | `Procedure.spec.target` is malformed, is declared on a builtin handler, names an unknown Schema, or its `id` is not a required string input property or its `version` not a number input property. | — |
