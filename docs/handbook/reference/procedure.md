...
`rowBindings` come from `x-mantle-ref` on the Procedure's input properties. An input property referencing a declared, non-`translates` Schema produces `{ collection, inputField, rowField }`, and Admin offers the operation from that collection's row menu with the field pre-filled and read-only. With the object form `x-mantle-ref: { schema, field }`, `rowField` is the declared `field`. With the string form, `rowField` is inferred: the target Schema's same-named property when it has one, otherwise the lone field of a single single-field unique index, otherwise the reserved `id` column. That inference is transitional (ADR-0029): when it binds a field other than `id`, Admin logs one warning per Procedure input at mount, naming the object form to declare. The next minor release reads the string form as `field: id`. A Procedure's operation `target` (declared, or implied by a builtin `update`, `delete`, `archive` or id-based `upsert`) is listed first and replaces any inference on the same input, since Admin prefills the first binding for a row's collection. Because such an operation now has a row binding, Admin offers it from the target collection's row menu rather than the global Operations page. Refs to unknown collections or to translation children produce no binding and no error.
