...
`rowBindings` come from `x-mantle-ref` on the Procedure's input properties. An input property referencing a declared, non-`translates` Schema produces `{ collection, inputField, rowField }`, and Admin offers the operation from that collection's row menu with the field pre-filled and read-only. With the object form `x-mantle-ref: { schema, field }`, `rowField` is the declared `field`. With the string form, `rowField` is inferred: the target Schema's same-named property when it has one, otherwise the lone field of a single single-field unique index, otherwise the reserved `id` column. That inference is transitional (ADR-0029); declare the object form when the bound field matters. Refs to unknown collections or to translation children produce no binding and no error.
