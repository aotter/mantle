...
`.mantle/cloud-artifact.json` containing the sealed RuntimePlan, the bundled
application module, static assets, SDK version, recipe version and lockfile
...
then verify handler exports and start the module with test bindings before it
can become a deployment candidate. A local artifact hash records local bytes;
it is not proof of a Cloud build or permission to deploy. The platform owns
the fixed runtime entry, production bindings, secrets, source commit and
candidate metadata. Project code receives only the tenant bindings permitted
