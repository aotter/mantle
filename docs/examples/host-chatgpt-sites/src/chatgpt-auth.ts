...
export function createChatGPTAuth(env: Env): AdminAuth {
  ...
  const auth: AdminAuth = {
    ...
    async getSession(request) {
      ...
      // Only the configured owner can bootstrap privilege; a later role revocation persists.
      ...
      const row=await db.prepare('SELECT * FROM sites_users WHERE id=?').bind(who.id).first<UserRow>();
      if(!row)return null;
      return {session:{id:`sites:${who.id}`},user:{...who,role:row.role,roleCurrent:true}};
    },
    getUserRole:roleFor,
