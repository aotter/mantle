...
|---|---|
| [`develop`](develop/SKILL.md) | `mantle:develop`: interview, local Worker, Admin OTP, then manifest, runtime, handler, adapter, validation, and MCP work. |
...
| [`plugin`](plugin/SKILL.md) | `mantle:plugin`: Core-owned marketplace workflow for plan-first capability installs. |
| [`theme`](theme/SKILL.md) | `mantle:theme`: visual workflow for an application that already has a frontend. |
...
| [`install`](install/SKILL.md) | Agent bringing a human: interview, author a local Worker, walk them into Admin with email OTP, or continue an existing project. |
...
|---|---|---|---|---|---|
| `develop` | existing project; interview, local Worker, Admin OTP, manifest, runtime, handler, adapter, or MCP work | interview then local D1 then Admin email OTP; four-atom model; adapter neutrality; no direct D1/KV/Postgres writes; no committed secrets; no overlay seed or invented homepage | performance harness; local MCP client; locale rules | project, plugin | — |
...
| `theme` | brand or visual direction when the application already has a frontend | repo-owned theme and UI contracts; do not invent a homepage | — | project, plugin | — |
| `update` | SDK upgrade or plugin lock review | never blindly overwrite user-owned code | — | project, plugin | — |
| `install` | new service, or opening an existing project | interview first; do not use the SDK checkout as the application; no push/deploy during local verification; no `mantle create` | author local Worker and Admin OTP; continue existing project | plugin | Creates a new project; nothing to project into an existing one. |
...
skill `mantle skills` should place in an application, and a skill that
...
Leftover launch files and plugin recipes are project context, not competing
contracts.
...
The skills target ADR-0007's "AI as primary author" thesis: agents reach these files by URL when the user invokes them by intent ("install mantle", "develop my Mantle service", "deploy"). No `/skill install` slash command is required — point the agent at a version tag or pass the version-matched markdown content directly.
