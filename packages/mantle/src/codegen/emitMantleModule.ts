...
function findIdentifierCollision(linked: LinkedManifestSet | RuntimePlan): Diagnostic | undefined {
  const groups = [
...
type NamedManifestEntry = {
  readonly manifest: { readonly metadata: { readonly name: string } };
  readonly source?: { readonly sourceId: string; readonly documentIndex: number; readonly path: string };
};

function values(
  entries: readonly NamedManifestEntry[] | Readonly<Record<string, NamedManifestEntry>>,
): readonly NamedManifestEntry[] {
