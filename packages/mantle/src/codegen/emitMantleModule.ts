...
export function emitMantleModule(request: EmitMantleModuleRequest): EmitMantleModuleResult {
  ...
  return {
    ...
    source: [
      ...
      "export type MantleEntry<TData extends Record<string, unknown>> = CoreEntry<TData>;",
      "/** Scalar members of one Schema field, the only values field reads accept. */",
...
function schemaLines(name: string, namespace: string): string[] {
  ...
  return [
    ...
    `          runtime.entries.readBySlug({ ...request, collection: ${JSON.stringify(name)} }) as Promise<MantleEntry<${data}> | null>,`,
    `        readByDataField: <F extends keyof ${data} & string>(request: Omit<ReadEntryByDataFieldArgs, "collection" | "field" | "value"> & { readonly field: F; readonly value: MantleFieldValue<${data}, F> }) =>`,
    `          runtime.entries.readByDataField({ ...request, collection: ${JSON.stringify(name)} }) as Promise<MantleEntry<${data}> | null>,`,
    `        readByDataFieldIn: <F extends keyof ${data} & string>(request: Omit<ReadEntriesByDataFieldInArgs, "collection" | "field" | "values"> & { readonly field: F; readonly values: readonly MantleFieldValue<${data}, F>[] }) =>`,
    `          runtime.entries.readByDataFieldIn({ ...request, collection: ${JSON.stringify(name)} }) as Promise<readonly MantleEntry<${data}>[]>,`,
    `        findManyByDataField: <F extends keyof ${data} & string>(request: Omit<FindManyEntriesByDataFieldArgs, "collection" | "field" | "value"> & { readonly field: F; readonly value: MantleFieldValue<${data}, F> }) =>`,
    `          runtime.entries.findManyByDataField({ ...request, collection: ${JSON.stringify(name)} }) as Promise<readonly MantleEntry<${data}>[]>,`,
