...
function emitManifestModule(manifests: readonly Manifest[], namespace: string): string {
  const views: Array<{ readonly manifest: ViewManifest; readonly index: number }> = [];
  const procedures: Array<{ readonly manifest: ProcedureManifest; readonly index: number }> = [];
  manifests.forEach((manifest, index) => {
    if (manifest.kind === "View") views.push({ manifest, index });
    if (manifest.kind === "Procedure") procedures.push({ manifest, index });
  });
  ...
  return [
    ...
    "  return {",
    "    views: {",
    ...views.flatMap(({ manifest: view, index }) => {
      const id = tsId(view.metadata.name);
      const requiredParams = (view.spec.params?.required?.length ?? 0) > 0;
      const requestType = view.spec.params
        ? `MantleViewOptions & { readonly params${requiredParams ? "" : "?"}: MantleGenerated.${namespace}.ViewParams_${id} }`
        : "MantleViewOptions";
      return [
        `      ${JSON.stringify(view.metadata.name)}: (request: ${requestType}${requiredParams ? "" : " = {}"}) =>`,
        `        runtime.executeView.execute<MantleGenerated.${namespace}.ViewRow_${id}>({`,
        `          view: manifest[${index}],`,
        "          ctx: request.ctx,",
        "          options: {",
