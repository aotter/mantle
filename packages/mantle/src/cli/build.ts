...
const manifestSource = async (root: string) => {
  const entries = (await readdir(root, { withFileTypes: true })).filter(entry => /\.ya?ml$/i.test(entry.name)).sort((a, b) => a.name.localeCompare(b.name));
  if (!entries.length || entries.length > 100) throw new Error(`${root}: expected 1–100 Manifest files`);
  const files: Record<string, string> = {};
  let total = 0;
  for (const entry of entries) {
    if (!entry.isFile()) throw new Error(`${entry.name}: only regular Manifest files are supported`);
    const bytes = await readFile(join(root, entry.name));
    total += bytes.length;
    if (bytes.length > 200_000 || total > 1_000_000) throw new Error(`${entry.name}: Manifest source exceeds Cloud limits`);
...
export async function runBuild(rawArgs: readonly string[]): Promise<number> {
  try {
    ...
    const source = await manifestSource(manifests);
    ...
    if (sha256(JSON.stringify(await manifestSource(manifests))) !== sourceDigest || sha256(await readFile(join(root, 'pnpm-lock.yaml'))) !== sha256(lockfile) ||
    ...
    const artifact = { version: 1, sdkVersion: sdk.version, recipe: { node: '22', pnpm: '9.15.0' },
      ...
      lockfileSha256: sha256(lockfile), manifests: source, plan: compiled.value, module, assets };
