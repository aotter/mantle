...
/**
 * Run a Core or application use case behind Mantle's stable HTTP diagnostic
 * boundary. Extension routes should use this instead of exposing raw errors or
 * inventing another status mapping.
 */
export async function runMantleUseCase<T>(
  ...
  try {
    ...
    // Adapter/storage errors can contain account IDs, query fragments, and
    // bucket names. Keep details in Worker logs, never on the wire.
