...
function LogicSidebar({ items, selectedId, search, onSearch, onSelect }: { items: LogicItem[]; selectedId: string; search: string; onSearch: (value: string) => void; onSelect: (id: string) => void }): React.ReactElement {
  const { language } = usePreferences();
  return (
    <aside className="flex h-full min-h-0 flex-col border-e bg-sidebar text-sidebar-foreground" aria-label={t(language, "logic.objects")}>
...
function LogicItemButton({ item, selected, onSelect }: { item: LogicItem; selected: boolean; onSelect: (id: string) => void }): React.ReactElement {
  const channel = item.kind === "Trigger" ? triggerChannel(item.model.source) : item.model.handler.kind;
  return (
    <button type="button" onClick={() => onSelect(item.id)} aria-pressed={selected} className={cn("mt-1 flex h-8 w-full items-center gap-2 rounded-md px-2 text-start hover:bg-sidebar-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sidebar-ring", selected && "bg-sidebar-accent font-medium text-sidebar-accent-foreground")}>
      <span className="min-w-0 flex-1 truncate font-mono text-xs">{item.model.name}</span>
      <span className="shrink-0 rounded border border-sidebar-border bg-sidebar-accent/50 px-1 py-0.5 font-mono text-[8px] font-medium uppercase text-sidebar-foreground/70">{channel}</span>
    </button>
  );
}
...
function triggerFacts(language: ReturnType<typeof usePreferences>["language"], model: DeveloperTriggerModel): Array<[string, string]> {
  const base: Array<[string, string]> = [
    ...
    [t(language, "developer.graph.fact.transport"), triggerChannel(model.source)],
  ];
