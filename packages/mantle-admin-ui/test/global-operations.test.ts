...
it("discovers global operations without collections and preserves failed input and successful output", async () => {
  ...
  try {
    ...
    await page.route("**/admin/api/**", async route => {
      ...
      const data = path === "/me" ? { id: "owner", role: "owner", login: "owner", image: null }
        ...
        : path === "/operations" ? { operations: [{ name: "inspect", title: "Inspect tenant", rowBindings: [], input: { type: "object", required: ["tenantId", "operationId"], properties: { tenantId: { type: "string", title: "Tenant ID" }, operationId: { type: "string", "x-mcp-hint": "idempotency-key" } } } }] }
        : {};
    ...
    await dialog.getByText("The operation ran but failed to complete.", { exact: true }).waitFor();
    ...
    const repeated = page.waitForResponse(response => response.url().endsWith("/operations/inspect") && response.status() === 200);
    await dialog.getByRole("button", { name: "Run", exact: true }).click();
    await repeated;
