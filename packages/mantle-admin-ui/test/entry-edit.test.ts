...
describe("stringFieldWidget", () => {
  it("uses only explicit UI and content-format declarations", () => {
    ...
    expect(stringFieldWidget({ type: "string", "x-mcp-hint": "code" }, null)).toBe("input");
    expect(stringFieldWidget({ type: "string", "x-mcp-hint": "code" }, "textarea")).toBe("textarea");
  });

  it("splits markdown, html, and richtext hints onto distinct widgets", () => {
    ...
    expect(stringFieldWidget({ type: "string", "x-mcp-hint": "richtext" }, null)).toBe("richtext");
    expect(stringFieldWidget({ type: "string", "x-mcp-hint": "richtext" }, "textarea")).toBe("richtext");
  });
});
